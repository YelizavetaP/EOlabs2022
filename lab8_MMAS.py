...
class AntColony(object):
    ...
    def __init__(self, distances, n_ants, n_best, n_iterations, decay, alpha=1, beta=1):
        ...
        self.t_max = 1
        self.t_min = 0.1
        self.p_dec = 1/len(self.pheromone)
    ...
    def run(self):
        ...
        while(self.n_iterations > 0):
            ...
            self.t_max = 1/(self.decay*shortest_path[1])
            self.t_min = (self.t_max*(1-self.p_dec))/(self.p_dec*self.n_ants)
    ...
    def spread_pheronome(self, all_paths, n_best, shortest_path):
        ...
        #помилюємо феромон на найкращому шляху
        for path, dist in sorted_paths[:n_best]:
            for move in path:
                if shortest_path != None:
                    if move in shortest_path[0]: #тільки якщо найкращ мураха використала відрізок
                        self.pheromone[move] += 1.0 / shortest_path[1] #1 -> Q
            for move in path:
                if self.pheromone[move] > self.t_max: self.pheromone[move] = self.t_max
                if self.pheromone[move] < self.t_min: self.pheromone[move] = self.t_min
    ...
    def pick_move(self, pheromone, dist, visited):
        # феромон на маршрутах до можливих міст з поточного, відстані до інших міст від поточного, вже відвідані міста


        pheromone = np.copy(pheromone)
        pheromone[list(visited)] = 0 #обнуляємо феромон на вже відвіданих містах
        ...
        if self.n_iterations > 1:
            n =  1/(self.n_iterations-1)

        self.p_dec = max(norm_row)**(n)

        move = np_choice(self.all_inds, 1, p=norm_row)[0] #обираємо 1 маршрут з (0, 25) з імовірностями з norm_row
...
distances =  np.array([
            	[np.inf,     645,	868,	125,	748,	366,	256,	316,	1057,	382,	360,	471,	428,	593,	311,	844,	602,	232,	575,	734,	521,	120,	343,	312,	396],
                ...
                [1057,	394,	148,	1182,	365,	1402,	811,	668,	np.inf,  	1199,	1379,	857,	977,	474,	1129,	739,	523,	1289,	1539,	333,	806,	1177,	706,	1292,	951],
                ...
                [602,	430,	706,	477,	477,	908,	346,	506,	523,	734,	896,	681,	800,	261,	664,	896,	np.inf,  	774,	1138,	190,	732,	662,	540,	883,    350],
...
cities = ['Вінниця', 'Дніпро','Донецьк','Житомир','Запоріжжя','Івано-франківськ',
            'Київ','Кропивницький','Луганськ','Луцьк','Львів','Миколаїв','Одеса','Полтава',
            'Рівне','Сімферополь','Суми','Тернопіль','Ужгород','Харків','Херсон',
            'Хмельницький', 'Черкаси','Чернівці','Чернигів']
...
trys = []
for i in range(10):
    ant_colony = AntColony(distances, 20, 1, 500, 0.05, alpha=2, beta=2)
    shortest_path = ant_colony.run()
    trys.append(shortest_path)

cities_path = []
trys = sorted(trys, key= lambda ind: ind[1])

for point in trys[0][0]:
    cities_path.append(cities[point[0]])

print('ACS')
print ("shorted_path: {}, \n Загальна відстань: {}".format(cities_path, trys[0][1] ))