...
class AntColony(object):
            ...
            for path, dist in all_paths:
                for move in path:
                    if move in shortest_path[0]: #тільки якщо найкращ мураха використала відрізок
                        self.pheromone[move] = (1-self.decay)*self.pheromone[move] + self.decay*(1.0 / shortest_path[1])
    ...
    def spread_pheronome(self, all_paths, n_best, shortest_path):
        ...
        for path, dist in sorted_paths[:n_best]:
            for move in path:
                self.pheromone[move] += 1.0 / dist #1 -> Q
    ...
    def pick_move(self, pheromone, dist, visited):
        ...
        q = random.random()
        if (q < self.q0):
            p_row = pheromone * (( 1.0 / dist) ** self.beta)
            move = np.where(p_row == max(p_row))[0][0]
        else:
            p_row = pheromone ** self.alpha * (( 1.0 / dist) ** self.beta)
            norm_p_row = p_row / p_row.sum()

            move = np_choice(self.all_inds, 1, p=norm_p_row)[0] #обираємо 1 маршрут з (0, 25) з імовірностями з norm_row
...
trys = []
for i in range(10):
    ant_colony = AntColony(distances, 20, 20, 1000, 0.05, alpha=2, beta=2)
